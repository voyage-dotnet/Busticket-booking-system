...
namespace BusTicketSystem.Web.Repositories
{
    public class AgencyRepository : IAgencyRepository
    {
        ...
        public async Task<List<Bus>> GetBusesByAgencyIdAsync(int agencyId)
        {
            return await _context.Buses
                .Include(b => b.Office)
                .Where(b => b.Office.AgencyId == agencyId)
                .OrderBy(b => b.BusId)
                .ToListAsync();
        }

        public async Task<List<Driver>> GetDriversByAgencyIdAsync(int agencyId)
        {
            return await _context.Drivers
                .Include(d => d.Office)
                .Where(d => d.Office.AgencyId == agencyId)
                .OrderBy(d => d.DriverId)
                .ToListAsync();
        }
    }
