...
public class PaymentService : IPaymentService
{
    ...
    public async Task<object> ProcessPaymentAsync(int customerId, PaymentRequestDTO dto)
    {
        ...
        var expectedAmount = booking.Trip!.Fare + 7.50m;
        if (dto.Amount != expectedAmount)
            throw new BadRequestException(
                $"Amount must match the total fare (including fees) of {expectedAmount}.");
    ...
    public async Task<object> CreateRazorpayOrderAsync(int customerId, CreateOrderDTO dto)
    {
        ...
        catch (Exception ex)
        {
            ...
            throw new Exception($"Razorpay Order Creation Failed: {string.Join(" --> ", messages)}. KeyId prefix: {keyId.Substring(0, 5)}...");
        }
    }
